...
def main() -> None:
    ...
    "$idevice_root/ffi/src/tunnel_provider.rs"\n  python3 "$BUILDER/scripts/patch_v17_rsd_core.py" \\
...
'''
    ...
    src = src.replace(
        "echo 'preflight=PASS'",
        "echo 'v17_fix=single TLS-record CDTunnel request + fragmented response carry + linked RSD core phased timeouts + retry pipeline'\n    echo 'preflight=PASS'",
    )
    src = src.replace(
